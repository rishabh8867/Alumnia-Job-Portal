...
function AlumniForm({ formData }) {
  ...
  const [eventGuests, setEventGuests] = useState({
    Marathon: 1,
    "Alumni Family Connect": 1,
    "Gala Dinner & Cultural Program": 1,
  });
  const [events, setEvents] = useState([]);
  const [totalCost, setTotalCost] = useState(0);
  ...
  useEffect(() => {
    ...
    events.forEach((event) => {
      if (event === "Marathon") {
        cost += eventCosts[event] * eventGuests.Marathon;
      } else if (event === "Alumni Family Connect") {
        cost += eventCosts[event] * eventGuests["Alumni Family Connect"];
      } else if (event === "Gala Dinner & Cultural Program") {
        cost +=
          eventCosts[event] +
          300 * (eventGuests["Gala Dinner & Cultural Program"] - 1);
      }
    });
    setTotalCost(cost);
  }, [events, eventGuests]);
    ...
    <div>
      <FormControl component="fieldset">
        ...
        <RadioGroup
          ...
          onChange={(e) => setIsBringingGuest(e.target.value === "yes")}
        >
